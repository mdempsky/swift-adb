...
/// ADBAuthProvider that stores the RSA key in the iOS Keychain.
final class KeychainAuth: ADBAuthProvider {
    static let shared = KeychainAuth()
    private static let tag = "soy.mdempsky.SwiftADB.adbkey"
    ...
    func publicKeyBytes() throws -> Data {
        ...
        return try ADBPublicKey.authData(pkcs1DER: externalRep(pub), label: "SwiftADB@iPhone")
    }
